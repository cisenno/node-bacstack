...
const encodeTarget = (buffer, target) => {
  ...
  if (target.net === 0xFFFF || !target.adr) {
    buffer.buffer[buffer.offset++] = 0;
      ...
      // Transform in hex
      let hex = parseInt(target.adr).toString(16);
      // Concat 0 if len hex is odd
      if (hex.length % 2 != 0) {
        hex = [0, hex].join('');
      }
      // Array of hex pairs
      let pairs_hex = hex.match(/[\dA-F]{2}/gi);
      // Add len of hex in buffer
      buffer.buffer[buffer.offset++] = pairs_hex.length;
      if (pairs_hex.length > 0) {
        for (let i = 0; i < pairs_hex.length; i++) {
          // Add each pair of hex in buffer
          // Buffer need value in integer, it will transform int to hex again.
          buffer.buffer[buffer.offset++] = parseInt(pairs_hex[i], 16);
      }
    }
  }
};
